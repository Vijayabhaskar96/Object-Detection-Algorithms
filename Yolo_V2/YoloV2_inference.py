...
class InferenceModel():
    def __init__(self,weights_path,S=13,B=5,C=20):
        ...
        self.output_labels = ["aeroplane", "bicycle", "bird", "boat", "bottle", "bus", "car", "cat", "chair", "cow",
                         "diningtable", "dog", "horse", "motorbike", "person", "pottedplant", "sheep", "sofa",
                         "train", "tvmonitor"]
        self.model = YoloV2_model.YoloV2Model(architechture = self.architechture, split_size=S, num_boxes=B, num_classes=C)
...
#         orig_img = Image.merge("RGB", (b, g, r))
        ...
        resized_tensor = ToTensor()(orig_img.resize(size=(416,416),resample=PIL.Image.BILINEAR)).float()
    ...
    def post_process(self, pred):
        ...
        anchor_boxes = torch.tensor([[0,0,1.3221, 1.73145],
                                     [0,0,3.19275, 4.00944],
                                     [0,0,5.05587, 8.09892],
                                     [0,0,9.47112, 4.84053],
                                     [0,0,11.2364, 10.0071]],device=pred.device)
        ...
        bw = exist_mask*anchor_boxes[:,2].view([1,1,1,-1,1]) * exist_mask*torch.exp(pred[...,2:3])
        bh = exist_mask*anchor_boxes[:,3].view([1,1,1,-1,1]) * exist_mask*torch.exp(pred[...,3:4])
    ...
    def get_bboxes(self, x, predictions, iou_threshold=0.5, threshold=0.4,S=13, B=5):
        ...
        for idx in range(len(x)):
            ...
            if len(bboxes)==0:
                continue
            nms_boxes_idxs = batched_nms(boxes = self.yolo_to_normal(bboxes_alone), scores=bboxes_conf,idxs=bboxes_idx,iou_threshold=iou_threshold)
    ...
    def draw_bb(self, source_img, c, bb):
        ...
        draw.text((xmin+3, ymin+1), c,fill=color,stroke_fill=(255,255,255),stroke_width=2, font=ImageFont.truetype("arial"))


filepath = "pretrained_yolov2-voc-model_weights.pth"
inference_pipeline=InferenceModel(weights_path=filepath)
result_pil_image,bboxes=inference_pipeline.predict(r"/content/VOCdevkit/VOC2007/JPEGImages/000017.jpg")
