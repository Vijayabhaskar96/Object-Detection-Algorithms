...
conv_without_bn_config = namedtuple("ConvWithoutBnConfig", ["kernel_size", "filters", "stride", "pad"])
repeat_res_block = namedtuple("RepeatWithResidual", ["blocks", "n"])
repeat_block = namedtuple("Repeat", ["blocks", "n"])
darknet_base_config = [[conv_config(3, 32, 1, 1), conv_config(3, 64, 2, 1),
...
small_scale_config = [
    ...
    [conv_config(3, 1024, 1, 1), conv_without_bn_config(1, 75, 1, 0)]]
medium_scale_config = [
    [repeat_block([conv_config(1, 256, 1, 0), conv_config(3, 512, 1, 1)], 2)],
    [conv_config(1, 256, 1, 0)],
    [conv_config(3, 512, 1, 1), conv_without_bn_config(1, 75, 1, 0)]]
large_scale_config = [
    repeat_block([conv_config(1, 128, 1, 0), conv_config(3, 256, 1, 1)], 3),
    conv_without_bn_config(1, 75, 1, 0)]
...
class DoubleConvWithResidual(nn.Module):
    def __init__(self,repeat_block,in_channels):
        ...
        self.conv1 = CNNBlock(in_channels,conv.filters,kernel_size=conv.kernel_size,stride=conv.stride,pad=conv.pad)
        in_channels = conv.filters
        conv = repeat_block.blocks[1]
        self.conv2 = CNNBlock(in_channels,conv.filters,kernel_size=conv.kernel_size,stride=conv.stride,pad=conv.pad)
...
def conv_upsample_block(in_channels,filters,kernel_size=1,stride=1,pad=0):
    conv = CNNBlock(in_channels=in_channels,filters=filters,kernel_size=kernel_size,stride=stride,pad=pad)
...
class YoloV3Tail(nn.Module):
    def __init__(self, small_part_config, medium_part_config, large_part_config):
        super(YoloV3Tail, self).__init__()
        self.num_anchors = 3
        self.num_attrib = 5 + 20 #for PASCAL VOC
        ...
        self.small_part_branch_net = conv_upsample_block(in_channels=512,filters=256,kernel_size=1,stride=1)
        ...
        self.medium_part_branch_net = conv_upsample_block(in_channels=256,filters=128,kernel_size=1,stride=1)
    ...
    def forward(self,small_out,medium_out,large_out):
        ...
        tail_small_out = tail_small_out.permute(0,2,3,1).view(b,w,h,self.num_anchors,self.num_attrib)
        ...
        tail_medium_out = tail_medium_out.permute(0,2,3,1).view(b,w,h,self.num_anchors,self.num_attrib)
        ...
        tail_large_out = tail_large_out.permute(0,2,3,1).view(b,w,h,self.num_anchors,self.num_attrib)
...
class YoloV2Loss(nn.Module):
    ...
    def __init__(self, anchor_boxes, S=13):
...
class YoloV3Model(pl.LightningModule):
    def __init__(self):
        ...
        self.tail = YoloV3Tail(small_scale_config,medium_scale_config,large_scale_config)
    ...
    def _calc_map(self, y, pred):
        pred_boxes = []
        target_boxes = []
        small_preprocessed_pred = self._preprocess(pred[0],self.anchor_boxes[6:9]/(416/13),S=13)
        medium_preprocessed_pred = self._preprocess(pred[1],self.anchor_boxes[3:6]/(416/26),S=26)
        large_preprocessed_pred = self._preprocess(pred[2],self.anchor_boxes[:3]/(416/52),S=52)
        
        pred_boxes, target_boxes = get_bboxes(
            y=y,
            predictions=(small_preprocessed_pred,medium_preprocessed_pred,large_preprocessed_pred),
            iou_threshold=0.45,
            threshold=0.005,
            S=[13,26,52],
            B=3,
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            mAP = self._calc_map(y, (pred_y[0].clone(),pred_y[1].clone(),pred_y[2].clone()))
...
if __name__ == "__main__":
    ...
    trainer = pl.Trainer(gpus=1,overfit_batches=1,checkpoint_callback=False ,max_epochs=10)
