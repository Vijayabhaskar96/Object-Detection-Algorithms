...
def get_bboxes(y, predictions, iou_threshold, threshold, S, B, device):
    all_pred_boxes = []
    all_true_boxes = []
    ...
    for idx in range(len(y[0])):
        ...
        true_bboxes2 = all_true_bboxes[all_true_bboxes[:, 1] > threshold]
        idx_arr = torch.full(
    ...
    if all_pred_boxes:
        ...
        x = []
    if all_true_boxes:
        y = torch.unique(torch.cat(all_true_boxes), dim=0).tolist()
    else:
        y = []
    return x, y
